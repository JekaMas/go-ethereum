...
func (bc *BlockChain) WriteBlockWithState(ctx params.ContextWithForkFlags, block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	bc.chainmu.Lock()
	defer bc.chainmu.Unlock()

	return bc.writeBlockWithState(ctx, block, receipts, state)
}
...
func (bc *BlockChain) writeBlockWithState(ctx params.ContextWithForkFlags, block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	root, err := state.Commit(ctx.GetForkFlag(params.IsEIP158Enabled))
	if err != nil {
		return NonStatTy, err
	}
	triedb := bc.stateCache.TrieDB()
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, []interface{}, []*types.Log, error) {
	...
	case err == consensus.ErrPrunedAncestor:
		...
		return bc.insertSideChain(ctx, block, it)
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next(ctx) {
		...
		if err := bc.validator.ValidateState(ctxWithBlock, block, statedb, receipts, usedGas); err != nil {
			bc.reportBlock(block, receipts, err)
			atomic.StoreUint32(&followupInterrupt, 1)
			return it.index, events, coalescedLogs, err
		}
		...
		substart = time.Now()
		status, err := bc.writeBlockWithState(ctxWithBlock, block, receipts, statedb)
		if err != nil {
			atomic.StoreUint32(&followupInterrupt, 1)
			return it.index, events, coalescedLogs, err
		}
...
func (bc *BlockChain) insertSideChain(ctx params.ContextWithConfig, block *types.Block, it *insertIterator) (int, []interface{}, []*types.Log, error) {
	var (
		externTd *big.Int
		current  = bc.CurrentBlock()
	)
