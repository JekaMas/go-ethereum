...
func (t *VMTest) newEVM(statedb *state.StateDB, vmconfig vm.Config) *vm.EVM {
	...
	context := vm.Context{
		ContextWithForkFlags: params.NewContextWithBlock(params.MainnetChainConfig, big.NewInt(0).SetUint64(t.json.Env.Number)),
	...
	return vm.NewEVM(context, statedb, vmconfig)
}
